...
The file is `neo4j-hotel-graph.dump`, a few megabytes. It is also in this repository at `static/neo4j-hotel-graph.dump` if you have already cloned it locally.
...
That is the same check `setup/verify_setup.py` runs later. It reads one hotel by name and compares the address rather than counting nodes, because a count is plausible at any value and a half-restored dump passes it.
