...
"""Prove every `verify` judgement in both directions.
...
Run them with:
    ...
    uv run --with pytest --with neo4j --with python-dotenv pytest tests/
"""
...
def test_current_interpreter_is_accepted() -> None:
    assert verify.python_problems() == []


def test_old_interpreter_is_rejected() -> None:
    problems = verify.python_problems((3, 9))
...
def test_importable_module_produces_no_problem() -> None:
    assert verify.import_problems((("json", "the standard library"),)) == []


def test_missing_module_names_its_distribution() -> None:
    problems = verify.import_problems(
...
def test_every_module_the_workshop_needs_is_declared() -> None:
    ...
    declared = {name for name, _ in verify.REQUIRED_MODULES}
...
def test_complete_settings_are_accepted() -> None:
    ...
    assert verify.settings_problems(env) == []
...
def test_each_required_setting_is_missed_when_absent(missing: str) -> None:
    env = {"NEO4J_URI": "neo4j+s://example", "NEO4J_PASSWORD": "secret"}
    env.pop(missing)
    problems = verify.settings_problems(env)
    assert len(problems) == 1
...
def test_real_identity_is_accepted() -> None:
    assert verify.identity_problems(GOOD_IDENTITY) == []


def test_absent_identity_is_rejected() -> None:
    assert verify.identity_problems(None) != []
    assert verify.identity_problems({}) != []


def test_short_account_id_is_rejected() -> None:
    identity = GOOD_IDENTITY | {"Account": "12345"}
    assert any("12 digits" in problem for problem in verify.identity_problems(identity))


def test_non_arn_caller_is_rejected() -> None:
    identity = GOOD_IDENTITY | {"Arn": "not-an-arn"}
    assert any("arn:aws" in problem for problem in verify.identity_problems(identity))
...
def test_an_unexpected_exception_becomes_a_reported_problem() -> None:
    ...
    problems = verify.run_check(explode)
...
def test_a_blocking_failure_stops_the_run(capsys, monkeypatch) -> None:
    ...
    monkeypatch.setattr(verify, "load_environment", list)
    monkeypatch.setattr(
        verify,
        "CHECKS",
        (
...
def test_a_clean_run_exits_zero(capsys, monkeypatch) -> None:
    monkeypatch.setattr(verify, "load_environment", list)
    monkeypatch.setattr(verify, "CHECKS", (("only", lambda: [], False),))
