...
These tools are for maintaining workshop content and publishing graph
artifacts. They are not part of any participant path; a participant needs only
`environment/verify.py`. Run commands from the repository root unless a script
says otherwise.
...
```bash
python tools/quality/check_repo.py
```

For the local maintenance test suite:

```bash
notebooks/.venv/bin/python -m pytest tests -q
```
