...
Run them with:
    ...
    uv run --with pytest pytest workfolder/tests/test_check_repo.py
"""
...
def fake_repo(tmp_path: Path, monkeypatch) -> Path:
    ...
    content = tmp_path / "workshop-content" / "content"
...
def page(fake_repo: Path, body: str, folder: str = "01-build-graph") -> Path:
    ...
    path = fake_repo / "workshop-content" / "content" / folder / "index.en.md"
