...
usage() {
  echo "Usage: workfolder/maintenance/release/run_additive_validation.sh [--retain] [candidate.dump] [output-dir]"
...
run_validator "$OUTPUT_DIR/prebuilt-source-reconciliation.log" \
  "$REPO_ROOT/workfolder/maintenance/release/validate_graph_amenities.py" --mode prebuilt
run_validator "$OUTPUT_DIR/prebuilt-candidate-validation.log" \
  "$REPO_ROOT/workfolder/maintenance/release/validate_prebuilt_candidate.py"

echo "Adding and reconciling the five held-out documents..."
run_validator "$OUTPUT_DIR/additive-operation.log" \
  "$REPO_ROOT/workfolder/maintenance/release/run_additive_validation.py" --output-dir "$OUTPUT_DIR"
