...
#
# Usage: workfolder/maintenance/release/build_prebuilt_graph.sh [--resume]
...
case "${1:-}" in
  ...
  -h|--help)
    echo "Usage: workfolder/maintenance/release/build_prebuilt_graph.sh [--resume]"
    echo
  ...
  *)
    ...
    echo "Usage: workfolder/maintenance/release/build_prebuilt_graph.sh [--resume]" >&2
    exit 2
    ;;
esac
if [[ $# -gt 1 ]]; then
...
cp "$REPO_ROOT/workfolder/maintenance/release/write_prebuilt_manifest.py" "$MANIFEST_WRITER"
...
cleanup() {
  ...
  if [[ "$BUILD_SUCCEEDED" == true ]]; then
    ...
    if docker volume inspect "$VOLUME" >/dev/null 2>&1; then
      ...
      echo "Resume with:" >&2
      echo "  NEO4J_PREBUILT_VOLUME='$VOLUME' NEO4J_PREBUILT_PASSWORD='$PASSWORD' workfolder/maintenance/release/build_prebuilt_graph.sh --resume" >&2
    fi
  fi
  rm -rf "$SCRATCH"
  rm -rf "$PREBUILT_SCRIPT_SNAPSHOT_DIR"
