...
| File | Field |
| --- | --- |
| `environment/vocareum/course.json` | `startercode.commit` |
| `environment/vocareum/lab.template` | `Metadata.SourceCommit` |

`aws-vocareum` compares them and refuses a mismatch. `environment/vocareum/bump_pin.py`
...
`site/content/setup/*` is **not** in that list. Those pages are
...
```bash
uv run environment/vocareum/bump_pin.py --check
```
...
not before. Walk `CONFIG.txt` and `environment/verify.py` the way a student
would.
